# Engineering Principles

> Starter principles — edit, remove, or add your own.
> Run `/forge:retro-note` to capture refinements as
> you work.

## KISS: Keep it simple

Prefer the simplest solution that works. Refactor
toward simplicity. Small, focused components over
large, complex ones.

## Do one thing and do it well

Components with limited scope are easier to design,
test, and maintain. Modular designs lead to cleaner
systems.

## Less is more

Do the least you can to get the job done. You can
always do more later. Avoid unnecessary work that
may be thrown away.

## Consistency is key

Reuse well-established idioms. Reduce the learning
curve for others. When approaches differ, converge
on one.

## Good enough is not good enough

Small inefficiencies multiply. Don't poll when you
can subscribe. Cache efficiently. Minimize
duplication. Take pride in optimal solutions.

## Inform intrinsically

Users learn best in context. Surface knowledge when
users are primed with what they are trying to do,
not in docs they won't read.

## Team Principles

### Lead with principles

Start with the "why" before the "how". Foundational
principles come first — specific technology details
follow once the reader has the mental model.

### Write brief, plain-spoken language

Short sentences. Common words. No jargon without
explanation. If a simpler word works, use it.

### Visual diagrams are essential

Diagrams are not decoration — they are a primary way
to communicate. Include visuals to make abstract
concepts concrete and relationships clear.

### Concepts before technology details

Introduce ideas and vocabulary before showing tool
usage. The reader should understand what a concept
is and why it matters before seeing a specific
implementation.
